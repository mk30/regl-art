  ...
  return regl({
    frag: glsl `
      ...
      #pragma glslify: hsl2rgb = require('glsl-hsl2rgb')
			...
			uniform float time;
			void main () {
				float h = (snoise(vec3(vpos,time))*0.5+0.5)
					 * (sin(time)*0.5+0.5) + (sin(time*0.2)*0.5+0.5);
				float l = pow(max(
					sin(vpos.x*128.0)*0.5+0.5,
					sin(vpos.y*128.0)*0.5+0.5
				),64.0)*(snoise(vec3(vpos*64.0,time*8.0))*0.5+0.5);
...
function bg () {
  return regl({
    frag: glsl `
      ...
      uniform float time;
      void main () {
        float h = (snoise(vec3(uv*8.0,time))*0.5+0.5) * 0.3 + 0.5;
        float l0 = pow(
          (snoise(vec3(uv*32.0,time*0.2))*0.5+0.5),
          16.0
        );
        vec3 c = hsl2rgb(h,1.0,l0)
          + vec3(0,1,0)*pow(
            (snoise(vec3(uv*0.5,time*0.5))*0.5+0.5),16.0)
          + vec3(1,0,1)*pow(
            (snoise(vec3(uv*0.5,time*2.0))*0.5+0.5),16.0)
          + vec3(0,1,1)*pow(
            (snoise(vec3(uv*0.2,time*4.0))*0.5+0.5),8.0)
        ;
